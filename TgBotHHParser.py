...
class DataBase:
    ...
    def request_BD(self, country_id):
        with sq.connect(f"{self.derectory}/areas.db") as base:
            cur = base.cursor()
...
async def command_start(message : types.Message):
    await bot.send_message(message.from_user.id, "Начал загрузку всех вакансий по указанным параметрам. Это должно занять не более 20 секунд. Как все будет готово я пришлю сообщение")
...
async def command_start(message : types.Message):
    await bot.send_message(message.from_user.id, f"{data_base.dict_country}\nДля того чтобы узнать коды регионов нужной страны,"
                                                 f" отошли сообщение формата /113 . Где 113 номер нужной тебе страны", reply_markup=kb_klient_parser)
